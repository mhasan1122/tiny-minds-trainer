...
import { DIFFICULTY_CONFIG, SHAPES } from '@/constants/gameData';
...
import { ShapeIcon } from '@/components/game/ShapeIcon';
import {
  useFonts,
  Nunito_700Bold,
  Nunito_800ExtraBold,
...
export default function ShapeMatchingGame() {
  ...
  const [targetColor, setTargetColor] = useState(SHAPE_COLORS[0]);
  ...
  const generateRound = useCallback(() => {
    const numOptions = Math.min(config.itemCount, SHAPES.length);
    const shuffledShapes = shuffleArray(SHAPES);
    const selectedOptions = shuffledShapes.slice(0, numOptions);
  ...
  const handleShapeSelect = (shape: Shape, index: number) => {
      ...
      >
        {!isComplete ? (
          <>
            ...
            <Text style={styles.instructionText}>Find the shape!</Text>
            ...
            >
              {targetShape && (
                <ShapeIcon shape={targetShape.id} size={120} color={targetColor} />
              )}
              <Text style={styles.targetName}>{targetShape?.name}</Text>
            </Animated.View>
            ...
            <View style={styles.optionsGrid}>
              {options.map((shape, index) => (
                ...
                >
                  <TouchableOpacity
                    testID={`shape-option-${shape.id}`}
                    onPress={() => handleShapeSelect(shape, index)}
          ...
          <View style={styles.completeContainer}>
            <Text style={styles.completeTitle}>Super!</Text>
            <StarRating stars={getStars()} size={60} animated />
            <Text style={styles.finalScore}>
              You matched {score} shapes!
            </Text>
            <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  playAgainButton: {
    backgroundColor: GAME_COLORS.purple,
    paddingVertical: 16,
