...
import { ArrowLeft, RotateCcw, Apple, Star as StarIcon, Heart, Circle } from 'lucide-react-native';
...
const ICONS = [
  { Component: Apple, name: 'apples' },
  { Component: StarIcon, name: 'stars' },
  { Component: Heart, name: 'hearts' },
  { Component: Circle, name: 'circles' },
];
...
export default function CountingGame() {
  ...
  const [iconColor, setIconColor] = useState(SHAPE_COLORS[0]);
  ...
  const generateRound = useCallback(() => {
    const maxCount = currentDifficulty === 'easy' ? 5 : currentDifficulty === 'medium' ? 7 : 10;
    ...
    const allOptions = shuffleArray([count, ...Array.from(wrongOptions)]);
    ...
    while (buttonScales.length < 4) {
      buttonScales.push(new Animated.Value(1));
    }
  }, [currentDifficulty, buttonScales]);

  useEffect(() => {
    generateRound();
  }, [generateRound]);

  const handleOptionSelect = (selectedCount: number, index: number) => {
    if (feedback) return;
  ...
  return (
    <View style={styles.container}>
      <View style={[styles.header, { paddingTop: insets.top + 16 }]}>
        ...
        <Text style={styles.headerTitle}>Counting Fun</Text>
      ...
      >
        {!isComplete ? (
            ...
            <View style={styles.optionsContainer}>
              {options.map((num, index) => (
                ...
                >
                  <TouchableOpacity
                    testID={`option-${num}`}
                    onPress={() => handleOptionSelect(num, index)}
          ...
          <View style={styles.completeContainer}>
            <Text style={styles.completeTitle}>Awesome!</Text>
...
const styles = StyleSheet.create({
  ...
  optionButton: {
    ...
    alignItems: 'center',
    justifyContent: 'center',
    shadowColor: GAME_COLORS.secondary,
    shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.3,
    shadowRadius: 8,
    elevation: 5,
